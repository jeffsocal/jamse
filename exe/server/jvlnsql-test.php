...
$ui_expr = $cli->getVar('e', 'runtest', 'experiment name');
...
$jvln_lh = $cli->getVar('l', 50, 'hits per spectrum return limit');
$jvln_quorum = $cli->getVar('nq', 1, 'quorum match number ');
$jvln_nmusthave = $cli->getVar('nm', 6, 'n top must have at least 1');
 ...
 */
// if (! is_false($data = $spec->filePsmsExist($file_id)))
...
foreach ($scans as $scan_n => $scan) {
    ...
    if (count($this_peaks) >= $jvln_npeaksmin) {
        ...
        if (count($jvln_data) > 0) {
            $jvln_data = jvln_hits_munge($jvln_data, $this_peaks, $this_pre_mass, $this_pre_z, $jvln_lh);

            if (is_false($jvln_data))
                continue;
...
$csv = new Delim();
$csv->writeCsvFile("/var/jvlnse/local/" . $ui_expr . "/" . $file_id . ".csv", $jvln_out);
