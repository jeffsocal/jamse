...
if(length(files) == 1) {
    ...
    cat(" write out performance       :")
    time_stamp <- format(Sys.Date(), format="%Y%m%d")
    d_list <- list()
    d_list$summary <- ds_summary
    d_list$psms <- ds_results
    
    out_name <- paste0(str_replace(ui_path, "\\/$", ""), "jvln_dscore-performance_", time_stamp)
    
    png(filename = paste0(out_name, "_estimates.png"), pointsize = 10, 
        width = 360, height = 360)
    print(g_estimates)
    dev.off()
    
    png(filename = paste0(out_name, "_distributions.png"), pointsize = 10, 
        width = 360, height = 360)
    print(g_distributions)
    dev.off()
    
    json_txt <- rjson::toJSON(d_list)
    write(json_txt, paste0(out_name, ".json"))
    cat(" done\n")
}
